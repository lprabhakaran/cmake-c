...
void to_json(Json &json, const CompileCommandPrintSettings &ccpSettings)
{
    ...
    json[JConsts::projectIncludeDirs] = ccpSettings.includeDirs;
    json[JConsts::standardIncludeDirs] = ccpSettings.standardIncludeDirs;
...
void from_json(const Json &json, CompileCommandPrintSettings &ccpSettings)
{
    ...
    ccpSettings.includeDirs = json.at(JConsts::projectIncludeDirs).get<PathPrint>();
    ccpSettings.standardIncludeDirs = json.at(JConsts::standardIncludeDirs).get<PathPrint>();
...
void to_json(Json &json, const LinkCommandPrintSettings &lcpSettings)
{
    ...
    json[JConsts::libraryDirs] = lcpSettings.libraryDirs;
    json[JConsts::standardLibraryDirs] = lcpSettings.standardLibraryDirs;
...
void from_json(const Json &json, LinkCommandPrintSettings &lcpSettings)
{
    ...
    lcpSettings.libraryDirs = json.at(JConsts::libraryDirs).get<PathPrint>();
    lcpSettings.standardLibraryDirs = json.at(JConsts::standardLibraryDirs).get<PathPrint>();
...
string getReducedPath(const string_view subjectPath, const PathPrint &pathPrint)
{
    ...
    {
        return string(subjectPath);
    }

    auto nthOccurrence = [](const string &str, const size_t nth) -> size_t {
        size_t count = 0;
    ...
    auto countCharacter = [](const string &str) -> size_t {
        size_t count = 0;
        for (size_t offset = str.find(slashc); offset != string::npos; offset = str.find(slashc, offset + 1))
        {
            ++count;
        }
        return count;
